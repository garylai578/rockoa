...
var im={
	...
	resize:function(){
		...
		if(o2){
			...
			var nt=(whd-hw)*0.5-20;if(nt<0)nt=5;
	...
	initbackd:function(ret){
		...
		nwjs.createtray('信呼-'+adminname+'', 1);
	...
	showagents:function(d){
		...
		s= '<div onclick="im.openagent('+d.id+')" class="lists">';
		s+='<table cellpadding="0" border="0" width="100%"><tr>';
	...
	openuserzl:function(id){
		...
		var s = '<div>';
		s+='<div align="center" style="padding:10px;"><img id="myfacess" src="'+d.face+'" height="80" width="80" style="border-radius:40px;border:1px #eeeeee solid">';
		if(id==adminid)s+='<br><a href="javascript:;" id="fupbgonet" onclick="im.upfaceobj.click()" style="font-size:12px">修改头像</a>';
		s+='</div>';
		s+='<div style="line-height:25px;padding:10px;padding-left:40px;"><font color=#888888>姓名：</font>'+d.name+'<br><font color=#888888>部门：</font>'+d.deptname+'<br><font color=#888888>职位：</font>'+d.ranking+'<br><font color=#888888>性别：</font>'+d.sex+'<br><font color=#888888>电话：</font>'+d.tel+'<br><font color=#888888>邮箱：</font>'+d.email+'</div>';
		s+='</div>';
		js.tanbody('userziliao',''+d.name+'', 250,350,{
			html:s
		});
		if(id==adminid){
			this.upfaceobj=new rockupload({inputfile:'upfacess',uptype:'image',
	...
	receivemesb:function(d){
		...
		if(lx == 'agent'){
			...
			ot 		= $('#agentstotal_'+gid+'');ots=ot.text();if(ots=='')ots='0';
			if(!ops)ots = parseFloat(ots)+1;
			ot.html(''+ots);
			this.changestotl('agentstotal_',1);
		}
	...
	indexsyscog:function(){
		...
		s+='</div>';
		js.tanbody('syscogshow','设置',270,200,{html:s,bodystyle:'padding:5px'});
	},
	searchss:function(){
...
var chatclass = function(opts){
	...
	this._init=function(){
		...
		if(this.type=='group'){
			...
			s+='<td nowrap><span onclick="js.changeuser(\'changecheckuser\',\'yq\',\''+this.num+'\');"><i class="icon-plus"></i>&nbsp;邀请</span></td>';
		}
		s+='</tr></table>';
		$('#chatheader_'+this.rand+'').html(s);
		chatobj[this.num]=this;
		
		if(typeof(uploadobj)=='undefined')uploadobj = new rockupload({
			inputfile:'allfileinput',
			onchange:function(d){
				me.changeup(d);
			},
			onprogress:function(f,per,evt){
				chatobj[this.chatnum].onprogresss(f,per,evt);
			},
			onsuccess:function(f,str,o1){
				chatobj[this.chatnum].upfilesuccess(f,str,o1);
			}
		});
	...
	this._contright=function(o1,e){
		...
		this.randmess = rnd;
		var d=[{name:'复制',lx:0},{name:'删除',lx:1},{name:'清除1月前记录...',lx:2}];
...
function agentclass(opts){
	...
	this.showdata=function(a){
		...
		if(count>0){
			s = '<div class="showblank" id="showblank_'+this.rand+'">共'+count+'条记录';
			...
			s+= '</div>';
			this.showobj.append(s);
			if(a.count==0)$('#showblank_'+this.rand+'').html('');
		}else{
