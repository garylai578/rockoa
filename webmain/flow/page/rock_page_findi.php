...
$(document).ready(function(){
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'抵票列表('+nowtabs.name+')',
				cont:'抵票列表('+nowtabs.name+')的列表的',
				explain:'订阅[抵票列表]的列表',
				objtable:a
			});
		},
		getacturl:function(act){
			return js.getajaxurl(act,'mode_findi|input','flow',{'modeid':modeid});
		},
		...
		daoru:function(){
			window.managelistfindi = a;
			addtabs({num:'daorufindi',url:'flow,input,daoru,modenum=findi',icons:'plus',name:'导入抵票列表'});
		},
...
//[自定义区域end]
	...
	js.initbtn(c);
	var a = $('#viewfindi_{rand}').bootstable(bootparams);
	c.init();
