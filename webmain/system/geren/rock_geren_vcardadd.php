...
$(document).ready(function(){
	var c={
		yulan:function(){
			var celstr = '姓名,单位名称,电话,手机号,邮箱,所在组,地址';
		...
		downss:function(){
			js.open('upload/base/vcardimport.xls');
		},
		insrtss:function(){
			var val = mobjs.val();
			mobjs.val(val+'	');
		...
		saveadd:function(){
			...
			js.setmsg('处理中...','', vis);
			js.ajax(js.getajaxurl('piliangadd','{mode}','{dir}'),{importcont:val},function(ds){
				if(ds.success){
					js.msg('success', ds.msg);
					closenowtabs();
					try{vcardgroupd.reload();}catch(e){}
				}else{
